...
class LxmertForQuestionAnsweringOutput(ModelOutput):
    ...
    pooled_outputs: Optional[Tuple[torch.FloatTensor]] = None
    language_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    vision_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    language_attentions: Optional[Tuple[torch.FloatTensor]] = None
    vision_attentions: Optional[Tuple[torch.FloatTensor]] = None
...
class LxmertXLayer(nn.Module):
    ...
    def cross_att(
        ...
        # Cross Attention
        with Timer(f'x_{self.layer_idx}_lang2vis_cross'):
        ...
        with Timer(f'x_{self.layer_idx}_vis2lang_cross'):
    ...
    def output_fc(self, lang_input, visual_input):
        # FC layers
        with Timer(f'x_{self.layer_idx}_lang_inter'):
            lang_inter_output = self.lang_inter(lang_input)
        with Timer(f'x_{self.layer_idx}_vis_inter'):
            visual_inter_output = self.visn_inter(visual_input)

        # Layer output
        with Timer(f'x_{self.layer_idx}_lang_out'):
            lang_output = self.lang_output(lang_inter_output, lang_input)
        with Timer(f'x_{self.layer_idx}_vis_out'):
...
class LxmertEncoder(nn.Module):
    def __init__(self, config):
        ...
        use_x_layers = config.use_x_layers
        if use_x_layers:
            self.use_x_layers = use_x_layers
            logger.info(f'use_x_layers={use_x_layers} from config ')
        else:
            self.use_x_layers = config.x_layers

        # Layers
        # Using self.layer instead of self.l_layer to support loading BERT weights.
        self.layer = nn.ModuleList([LxmertLayer(config) for _ in range(self.num_l_layers)])
        self.x_layers = nn.ModuleList([LxmertXLayer(config, i) for i in range(self.num_x_layers)])
    ...
    def forward(
        ...
        with Timer('enc_cross'):
            for i in range(self.use_x_layers):
                layer_module = self.x_layers[i]
            # for layer_module in self.x_layers:
                with Timer(f'enc_cross_{i}'):
                ...
                if cross_encoder_attentions is not None:
                    x_attn = x_outputs[2]
                    cross_encoder_attentions = cross_encoder_attentions + (x_attn,)
                    if i == 0 and 0 < self.top_k_ratio < 1:
                        # only process 1st layer
                        b = visual_feats.size(0)
                        n_grids = visual_feats.size(1)
                        h = visual_feats.size(2)
                        top_k = int(self.top_k_ratio * n_grids)
                        # select top k image grids based on cross_attn
                        #  apply soft pruning/masking during training,
                        #  mask out visual_feats
                        top_val, top_idx = torch.topk(
                            x_attn.mean(1).sum(1), top_k, dim=-1)
                        t_idx = top_idx.unsqueeze(2).expand(b, top_k, h)
                        new_vis = visual_feats.gather(1, t_idx)
                        visual_feats = new_vis
                        # if self.training:
                        #     pass
                        # else:
                        #     # apply hard pruning during inference,
                        #     #  actually reduce visual_feats
                        #     pass
...
class LxmertModel(LxmertPreTrainedModel):
    def __init__(self, config):
        ...
        self.use_x_layers = config.use_x_layers
    ...
    def forward(
        ...
        lang_outputs = ()
        pooled_outputs = ()
        for x_layer in range(-self.use_x_layers, 0, 1):
            lang_output = language_hidden_states[x_layer]
            pooled_output = self.pooler(lang_output)
            lang_outputs = lang_outputs + (lang_output,)
            pooled_outputs = pooled_outputs + (pooled_output,)

        if not return_dict:
            return (lang_outputs, visual_output, pooled_outputs) + hidden_states + all_attentions

        return LxmertModelOutput(
            pooled_output=pooled_outputs,
            language_output=lang_outputs,
...
class LxmertForQuestionAnswering(LxmertPreTrainedModel):
    def __init__(self, config):
        ...
        # self.answer_head = LxmertVisualAnswerHead(config, self.num_qa_labels)
        # self.answer_head = nn.Linear(config.hidden_size, self.num_qa_labels)
        if config.single_ans_head:
            num_ans_heads = 1
        else:
            num_ans_heads = config.x_layers
        self.answer_heads = nn.ModuleList(
            [LxmertVisualAnswerHead(config, self.num_qa_labels)
             for _ in range(num_ans_heads)]
        )
        # Weight initialization
        self.init_weights()
    ...
    def forward(
        ...
        pooled_outputs = lxmert_output[2]
        losses = ()
        answer_scores = ()
        # loss = None
        # loss = 0
        # loss_w = 0
        if self.config.single_ans_head:
            use_layers = [-1]
        else:
            use_layers = range(self.use_x_layers)
            
        for x_layer in use_layers:
            answer_head = self.answer_heads[x_layer]
            answer_score = answer_head(pooled_outputs[x_layer])
            answer_scores = answer_scores + (answer_score,)
            if labels is not None:
                layer_loss = self.loss(answer_score.view(-1, self.num_qa_labels), labels)
                losses = losses + (layer_loss, )
        # answer_score = self.answer_head(pooled_output)
        if not return_dict:
            output = (answer_scores,) + lxmert_output[3:]
            return (losses,) + output if losses is not None else output

        return LxmertForQuestionAnsweringOutput(
            loss=losses,
            question_answering_score=answer_scores,
