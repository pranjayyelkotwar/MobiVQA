...
class VqaAccuracy(pl_metrics.Accuracy):
    def __init__(self,
        ...
        if not 0 < threshold < 1:
            raise ValueError(f"The `threshold` should be a float in the (0,1)"
                             f" interval, got {threshold}")
...
def compute_vqa_accuracy(gt_data, predictions):
    gt = {i['question_id']: i['label'] for i in gt_data}
    scores = 0
    for pred_i in predictions:
        qid = pred_i['question_id']
        ans = pred_i['answer']
        label = gt[qid]
        scores += label.get(ans, 0)
    accuracy = scores / len(predictions) * 100
    return accuracy
